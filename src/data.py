...
with open('./src/assets/data/MyBerlinTop10.csv', encoding='latin1') as data_file:
...
marathon_df = marathon_df_0.copy()
marathon_df["tps_fin"] = marathon_df["tps_fin"].astype(str).str.extract(r'(\d{2}:\d{2}:\d{2})')[0]

marathon_df["tps_fin"] = pd.to_timedelta(marathon_df["tps_fin"]).dt.total_seconds()
